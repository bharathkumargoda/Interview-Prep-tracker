...
app.get("/topics", requireAuth, (req, res) =>
  Topic.find({}).then(function (result) {
    //console.log(result);
    res.render("topics", { topicArray: result });
  })
);
app.get("/topics/*", requireAuth, (req, res) =>
  Topic.findOne({ name: decodeURIComponent(req.url.substring(8)) }).then(
    function (result) {
      //console.log(decodeURIComponent(req.url.substring(8)));
      //console.log('val of result:', result);
      Question.find({ topicname: result._id, approved: true }).then(function (
        result1
      ) {
  ...
  if (decodeURIComponent(req.url).split("/").length == 4) {
    ...
    Experience.findOne({ _id: decodeURIComponent(req.url).split("/")[3] }).then(
      function (result) {
        ...
        res.render("fullexperience", {
          experience: result,
          companyname: decodeURIComponent(req.url).split("/")[2],
        });
      }
    );
  } else {
    Company.findOne({ name: decodeURIComponent(req.url.substring(12)) }).then(
      function (result) {
        ...
        //console.log("val of result: ",result);
        Experience.find({ companyname: result._id, approved: true }).then(
          function (result1) {
