...
const handleErrorsQuestion = (err) => {
  ...
  // incorrect link
  if (err.message.includes("Please enter a link")) {
    errors.link = "Please enter a link";
  }
  // incorrect topicname
  if (err.message.includes("No Topic with this name")) {
    errors.topicname = "No Topic with this name";
  }
  if (err.code === 11000) {
    errors.link = "this question is already suggested";
    return errors;
  }
  return errors;
};
...
module.exports.questions_post = async (req, res) => {
  ...
  try {
    ...
    if (!topic) {
      throw Error("No Topic with this name");
    } else {
